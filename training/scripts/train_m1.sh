...
echo "Launching M1 on physical GPU(s): $CUDA_VISIBLE_DEVICES ($NUM_GPUS process(es))"
...
OUTPUT_DIR="${OUTPUT_DIR:-$REPO_ROOT/training_outputs/m1_data_only}"

export TOKENIZERS_PARALLELISM=false
export PYTHONPATH="$REPO_ROOT${PYTHONPATH:+:$PYTHONPATH}"
torchrun --standalone --nproc_per_node="$NUM_GPUS" -m training.train \
  --config "$REPO_ROOT/training/configs/m1_data_only.json" \
  --data-root "$DATA_ROOT" \
